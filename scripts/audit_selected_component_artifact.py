...
def audit(*, artifact: Path, policy_path: Path) -> dict[str, Any]:
    ...
    checks = {
        ...
        "planetary_msc_relations_promoted_as_stellar": "SELECT count(*) FROM msc_relation_evidence_projection WHERE regexp_matches(lower(coalesce(json_extract_string(quality_json,'$.Comment'),'')), '(^|[^a-z])(exoplanet|planet|planetary)([^a-z]|$)') AND projection_status NOT IN ('context_only_planetary_relation_evidence','invalid_self_relation_evidence')",
        ...
        "planetary_context_msc_orbits_selectable": "SELECT count(*) FROM msc_orbital_solution_projection p JOIN msc_orbit_solution_bindings b USING(orbit_binding_id) WHERE b.binding_status='context_only_planetary_relation' AND p.projection_status='eligible_for_quantity_selection'",
        ...
        "planetary_context_sb9_evidence_selectable": "SELECT (SELECT count(*) FROM sb9_stellar_parameter_projection p JOIN sb9_parameter_set_bindings b USING(parameter_set_binding_id) WHERE b.binding_status='context_only_planetary_relation' AND p.projection_status='eligible_for_quantity_selection') + (SELECT count(*) FROM sb9_classification_projection p JOIN sb9_relation_bindings b USING(relation_binding_id) WHERE b.binding_status='context_only_planetary_relation' AND p.projection_status='eligible_for_quantity_selection') + (SELECT count(*) FROM sb9_orbital_solution_projection p JOIN sb9_relation_bindings b USING(relation_binding_id) WHERE b.binding_status='context_only_planetary_relation' AND p.projection_status='eligible_for_quantity_selection')",
    ...
    msc_observed = {
        ...
        "relations_planetary_context": msc_relations.get("context_only_planetary_relation_evidence", 0),
        ...
        "orbits_planetary_context": msc_orbits.get("context_only_planetary_relation", 0),
    ...
    sb9_observed = {
        ...
        "relations_planetary_context": sb9_relations.get("context_only_planetary_relation", 0),
