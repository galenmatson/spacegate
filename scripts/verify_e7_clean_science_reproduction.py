#!/usr/bin/env python3
"""Reproduce clean E7 science projections and compare canonical artifact hashes."""
...
import verify_e7_clean_science as verifier


DEFAULT_REPORT = Path("/data/spacegate/state/reports/evidence_lake_v2/e7_clean_science_reproduction.json")
...
def reproduce(
    *,policy_path:Path,state_dir:Path,foundation_root:Path,artifact_root:Path,
    ...
    scratch=Path(tempfile.mkdtemp(prefix="e7-clean-science-reproduction-",dir=scratch_parent))
    try:
        rebuilt=compiler.compile_science(
            policy_path,state_dir,foundation_root,scratch/"artifacts",
        ...
        byte_exact=sorted(key for key,spec in reference_products.items() if spec.get("determinism")=="byte_exact")
        databases=sorted(key for key,spec in reference_products.items() if spec.get("determinism")=="logical_tables")
        differing_byte_exact=sorted(key for key in byte_exact if reference_products.get(key)!=rebuilt_products.get(key))
        differing_databases=sorted(key for key in databases if reference_products.get(key)!=rebuilt_products.get(key))
        checks={
            ...
            "projection_counts_match":rebuilt.get("projection_table_counts")==reference.get("projection_table_counts"),
    ...
    scratch_removed=not scratch.exists(); checks["scratch_removed"]=scratch_removed
    failures=[key for key,value in checks.items() if not value]
    usage=resource.getrusage(resource.RUSAGE_SELF)
    return {
        "schema_version":"spacegate.e7_clean_science_reproduction.v1",
        "generated_at":datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "build_id":build_id,"status":"pass" if not failures else "fail",
        ...
        "scratch_removed":scratch_removed,"rebuild_timing":rebuild_timing,
        "total_timing":{
