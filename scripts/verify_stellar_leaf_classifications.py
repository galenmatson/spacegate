    ...
    try:
        ...
        invalid_rows = scalar(
            ...
            where classification_value not in {tuple(sorted(VALID_CLASSES))}
               ...
               or projection_version <> 'stellar_leaf_display_classification_v1'
            """,
        )
        membership = con.execute(
            ...
            with expected as (
              ...
              join core.systems s on s.wds_id = n.wds_id
        ...
        for wds_id, expected in goldens.items():
            observed = [
                ...
                for row in con.execute(
                    """
                    select l.classification_value
                    from stellar_leaf_display_classifications l
                    join core.systems s using (system_id)
                    where s.wds_id = ?
                    order by l.hierarchy_node_key
                    """,
                    [wds_id],
                ).fetchall()
            ]
            passed = Counter(observed) == Counter(expected)
            if not passed:
                failures.append(f"golden:{wds_id}")
            golden_results[wds_id] = {"passed": passed, "expected": expected, "observed": observed}
        ...
        report = {
            "schema_version": "stellar_leaf_display_classification_verification_v1",
