...
if [[ "${SPACEGATE_ENABLE_GAIA_BACKBONE:-0}" == "1" ]]; then
  ...
  "$ROOT_DIR/scripts/catalogs.sh" \
    "${catalog_args[@]}" \
    "$@"
  echo "Gaia differential mode: backbone=${GAIA_BACKBONE_DELTA_MODE}@${GAIA_BACKBONE_DELTA_MAX_AGE_HOURS}h classprob=${GAIA_CLASSPROB_DELTA_MODE}@${GAIA_CLASSPROB_DELTA_MAX_AGE_HOURS}h nss=${GAIA_NSS_DELTA_MODE}@${GAIA_NSS_DELTA_MAX_AGE_HOURS}h"
