...
E7 clean extended-object build `c203e4f451890660ec02086a` is also unserved.
Its identity, selected geometry/search, 1,850 cluster distances, and 59
relation-bound distances are verified. HD 97472 remains an explicit missing
identity and 19 ineligible-scope cluster claims remain rejected. Runtime
...
Materializer v5 additionally requires the inner-first recursive tree policy and
source-group structural membership contract. Earlier build-keyed scene
artifacts are intentionally incompatible and regenerate rather than preserving
a flat pre-repair tree.
...
the legacy label bridge. These rows define source-scoped presentation
barycenters only; clients must not treat them as canonical containment or
...
- Missing public-read artifacts return a visible `503 public_read_unavailable`.
  The legacy DuckDB consumer is reachable only when
  `SPACEGATE_PUBLIC_READ_COMPATIBILITY_FALLBACK=true` is deliberately set for
  migration diagnostics; normal containers leave it disabled.
