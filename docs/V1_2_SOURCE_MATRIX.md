...
| WDS | broad visual multiplicity coverage | weak exact IDs; strong coordinate/discoverer designations | `review_required` | Valuable breadth source, but matches to core require confidence-scored crossmatching rather than exact joins; current prototype adds an optional CDS XMatch (`WDS`->`Gaia DR3`) bridge, still default-off pending stronger physical-consistency gating |
...
2. MSC for explicit higher-order hierarchy (approved optional; default-off policy).
