...
- `$SPACEGATE_BULK_DIR` (`/space/spacegate` on Photon): bulk research
...
The internal NVMe-backed `/space/spacegate` is the active bulk tier. The older
USB-backed `/mnt/space/spacegate` is a legacy/secondary tier. Anything in bulk
storage that matters to scientific traceability
