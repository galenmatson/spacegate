...
import {
  CAMERA_KEYBIND_CHANGE_EVENT,
  CAMERA_KEYBIND_SCHEMES,
  CAMERA_KEYBIND_STORAGE_KEY,
  cameraMovementToken,
  isCameraKeyboardInputTarget,
...
const SYSTEM_WORLD_UP = new THREE.Vector3(0, 1, 0);
...
function bodyContrastRadiusRsun(star) {
  const radius = fieldRecord(star?.fields, "radius_rsun");
  const numericRadius = Number(radius?.value);
  const status = String(radius?.status || "").toLowerCase();
  if (Number.isFinite(numericRadius) && numericRadius > 0 && !["assumed", "missing"].includes(status)) {
    return numericRadius;
  }
  const bodyClass = stellarBodyClass(star);
  if (bodyClass === "white_dwarf") return 0.012;
  if (["neutron_star", "pulsar", "magnetar"].includes(bodyClass)) return 0.00002;
  const classLetter = spectralClassLetter(visualStellarClassValue(star));
  return ({ O: 10, B: 4.5, A: 1.8, F: 1.3, G: 1, K: 0.78, M: 0.35, L: 0.12, T: 0.1, Y: 0.08 })[classLetter]
    || (Number.isFinite(numericRadius) && numericRadius > 0 ? numericRadius : 0.55);
}

function physicalLabelPlacementForRecord(layout, record) {
  if (!(layout instanceof Map)) return null;
  const keys = [
    record?.render_key,
    record?.orbit_key,
    record?.tree_node_key,
    record?.source_component_key,
    record?.stable_component_key,
    record?.stable_object_key,
    record?.key,
    record?.source?.stable_component_key,
    record?.source?.stable_object_key,
  ].filter(Boolean).map(String);
  return keys.map((key) => layout.get(key)).find(Boolean) || null;
}
...
function orbitPresentationScaleDefinition(stars = [], planets = [], visualScale = DEFAULT_VISUAL_SCALE) {
  const policy = visualScale.planet_orbit_radius || DEFAULT_VISUAL_SCALE.planet_orbit_radius;
  const orbitValues = planets
    .map((planet) => Number(planet?.orbitAu ?? numericField(planet?.fields, "semi_major_axis_au")))
    .filter((value) => Number.isFinite(value) && value > 0);
  const habitableValues = stars
    .map((star) => habitableZoneBoundsAu(star)?.outerAu)
    .filter((value) => Number.isFinite(value) && value > 0);
  const fallbackAu = Math.max(0.0001, Number(policy.fallback_au || 0.08));
  const maxOrbitAu = Math.max(fallbackAu, 0.1, ...orbitValues, ...habitableValues);
  const outerSceneRadius = Number(policy.min_scene || 0.75) + Number(policy.span_scene || 2.7);
  return {
    fallbackAu,
    maxOrbitAu,
    localSceneUnitsPerAu: outerSceneRadius / maxOrbitAu,
    logInnerSceneRadius: Number(policy.min_scene || 0.75),
    logSpanScene: Number(policy.span_scene || 2.7),
  };
}

function logarithmicOrbitScaleTicks(definition, count = 4) {
  const fallbackAu = Number(definition?.fallbackAu);
  const maxOrbitAu = Number(definition?.maxOrbitAu);
  if (!Number.isFinite(fallbackAu) || fallbackAu <= 0 || !Number.isFinite(maxOrbitAu) || maxOrbitAu <= 0) return [];
  const denominator = Math.max(0.0001, Math.log1p(maxOrbitAu / fallbackAu));
  return Array.from({ length: Math.max(2, count) }, (_, index) => {
    const fraction = index / Math.max(1, count - 1);
    const valueAu = fallbackAu * Math.expm1(denominator * fraction);
    return {
      fraction,
      valueAu: index === 0 ? 0 : valueAu,
      label: index === 0 ? "0 AU" : physicalScaleReadout(valueAu)?.primary,
    };
  });
}
...
function orbitHoverPayload(orbit) {
  ...
  const physicalStatusField = {
    ...
    notes: physicalExtent.applicability === "physical"
      ...
      : (coherence.status === "rejected"
        ? `The accepted period and stored axis imply an implausible total mass (${formatNumber(coherence.implied_total_mass_msun, 2)} solar masses), so this axis is excluded from physical scaling.`
...
function StarSphere({ star, position = [0, 0, 0], showLabels = true, selectedObjectId = "", targetRegistryRef = null, onHover, onSelect, onFocus }) {
  ...
  const calloutSide = Number(star.physical_label_side)
    || (hashUnit(star.render_key || star.key || star.display_name || star.name, "physical-label-side") < 0.5 ? -1 : 1);
  ...
  return (
    <group ref={groupRef} position={position}>
      ...
      <SceneLabel
        ...
        visible={showLabels && (!physicalMarker || star.physical_label_visible !== false)}
...
function TreeOrbitGuide({ spec, groupRefSetter, showOrbits = true, selectedObjectId = "", onHover, onSelect, onFocus }) {
  ...
  const unavailableCalloutSide = Number(spec.orbit.physical_label_side)
    || (hashUnit(spec.orbit.orbit_key || spec.orbit.display_name, "physical-label-side") < 0.5 ? -1 : 1);
      ...
      {spec.orbitRadius <= 0 && (
        <group {...handlers} userData={{ hoverPayload: orbitPayload }} data-testid="system-preview-physical-orbit-unavailable">
          ...
          <SceneLabel
            ...
            scale={0.72}
            maxLength={34}
            visible={spec.orbit.physical_label_visible !== false}
...
function SubsystemMarker({ subsystem, center = [0, 0, 0], groupKeys = [], groupMotionSpecs, layout, simClockRef, running = true, speedMultiplier = 1, scaleMode = "structure", showLabels = true, selectedObjectId = "", targetRegistryRef = null, onHover, onSelect, onFocus }) {
  ...
  const calloutSide = Number(subsystem.physical_label_side)
    || (hashUnit(subsystem.render_key || subsystem.key || subsystem.display_name || subsystem.name, "physical-label-side") < 0.5 ? -1 : 1);
  ...
  return (
    <group ref={groupRef} position={center} data-testid="system-preview-subsystem-marker">
      ...
      <SceneLabel
        ...
        color="#b7f3ff"
        scale={0.72}
        visible={showLabels && (!physicalMarker || subsystem.physical_label_visible !== false)}
        maxLength={physicalMarker ? 34 : 24}
        callout={physicalMarker}
        calloutSide={calloutSide}
...
function PlanetObject({ planet, orbitRadius, color, center = [0, 0, 0], motionGroupKeys = [], groupMotionSpecs, layout, treeContext = null, treeHostBodyKey = null, simClockRef, running = true, speedMultiplier = 1, showLabels = true, selectedObjectId = "", targetRegistryRef = null, onHover, onSelect, onFocus }) {
  ...
  const calloutSide = Number(planet.physical_label_side)
    || (hashUnit(planet.render_key || planet.key || planet.display_name || planet.name, "physical-label-side") < 0.5 ? -1 : 1);
  ...
  return (
    <group ref={groupRef} position={planet.physical_scale_unavailable ? addVector(center, [0.16, 0.12, 0]) : addVector(center, orbitalPositionFromMeanAnomaly(phaseRad, orbitRadius, eccentricity, inclinationRad))}>
      ...
      <SceneLabel
        ...
        color="#d7efff"
        scale={0.72}
        visible={showLabels && (!physicalMarker || planet.physical_label_visible !== false)}
...
function RendererResourceMetrics({ focusGraph = null, focusKey = "", sceneUnitsPerAU = null, compressedPhysicalRoot = false, physicalLabelCount = 0 }) {
  ...
  useFrame(({ clock }) => {
    if (clock.elapsedTime - lastSampleRef.current < 0.5) return;
    ...
    canvas.dataset.rendererDrawCalls = String(gl.info.render.calls || 0);
    canvas.dataset.rendererTriangles = String(gl.info.render.triangles || 0);
    canvas.dataset.physicalFocusKey = String(focusKey || "");
...
function CameraControls({ resetToken = 0, scaleMode = "structure", focusRequest = null, targetRegistryRef = null, keybindScheme = "wasd" }) {
  const { camera, gl, invalidate } = useThree();
  const controlsRef = React.useRef(null);
  const keysRef = React.useRef(new Set());
  const motionVectorsRef = React.useRef({
  ...
  const activeKeybind = CAMERA_KEYBIND_SCHEMES[normalizeCameraKeybindScheme(keybindScheme)] || CAMERA_KEYBIND_SCHEMES.wasd;
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event) => {
      if (isCameraKeyboardInputTarget(event.target)) return;
      const token = cameraMovementToken(event);
      if (!movementKeys.has(token)) return;
      event.preventDefault();
      event.stopPropagation();
      keysRef.current.add(token);
      invalidate();
    };
    const handleKeyUp = (event) => {
      const token = cameraMovementToken(event);
      if (!movementKeys.has(token)) return;
      keysRef.current.delete(token);
    };
    const clearKeys = () => keysRef.current.clear();
    canvas.tabIndex = 0;
    canvas.setAttribute("aria-label", `Interactive system simulation. ${activeKeybind.hint}; arrow keys also move the camera and Shift boosts speed.`);
    ...
    canvas.dataset.simulationKeybindScheme = activeKeybind.id;
    canvas.dataset.simulationKeybindHint = activeKeybind.hint;
    ...
    return () => {
      ...
      delete canvas.dataset.simulationKeybindScheme;
      delete canvas.dataset.simulationKeybindHint;
      delete canvas.dataset.simulationKeyboardMotion;
      canvas.removeAttribute("aria-label");
      canvas.removeAttribute("aria-keyshortcuts");
    };
  }, [activeKeybind, gl, invalidate]);
  ...
  useFrame((_, delta) => {
    if (controlsRef.current) {
      ...
      const { direction, strafe, movement, offset } = motionVectorsRef.current;
      movement.set(0, 0, 0);
      camera.getWorldDirection(direction).normalize();
      strafe.crossVectors(direction, SYSTEM_WORLD_UP).normalize();
      ...
      if (keys.has(activeKeybind.up)) movement.add(SYSTEM_WORLD_UP);
      if (keys.has(activeKeybind.down)) movement.sub(SYSTEM_WORLD_UP);
      if (movement.lengthSq() > 0 && !transitionRef.current) {
        const target = controlsRef.current.target;
        const targetDistance = Math.max(controlsRef.current.minDistance, camera.position.distanceTo(target));
        const baseSpeed = clampNumber(
          targetDistance * 0.65,
          activeScaleMode === PHYSICAL_SCALE_MODE ? 0.01 : 0.04,
          activeScaleMode === PHYSICAL_SCALE_MODE ? 40 : 14,
        );
        movement.normalize().multiplyScalar(baseSpeed * (keys.has("shift") ? 2.6 : 1) * Math.min(0.05, delta));
        camera.position.add(movement);
        offset.copy(camera.position).sub(target);
        if (offset.lengthSq() < controlsRef.current.minDistance ** 2) {
          if (offset.lengthSq() < 0.00000001) offset.copy(direction).multiplyScalar(-1);
          camera.position.copy(target).add(offset.setLength(controlsRef.current.minDistance));
        }
        gl.domElement.dataset.simulationKeyboardMotion = "active";
        invalidate();
      } else {
        gl.domElement.dataset.simulationKeyboardMotion = "idle";
      }
...
function ViewportScaleReporter({ enabled = false, sceneUnitsPerAU = null, mode = "", targetRegistryRef = null, focusTargetId = "", onChange = null }) {
  const { camera, size } = useThree();
  ...
  useFrame(({ clock }) => {
    ...
    const signature = [mode, lengthAu?.toPrecision(5), widthPx.toFixed(1), viewWidthAu.toPrecision(5)].join(":");
...
function NavigationTargetReporter({ targets = [], targetRegistryRef = null, sceneUnitsPerAU = null, onChange = null }) {
  const { camera, size } = useThree();
  const lastSampleRef = React.useRef(0);
  ...
  useFrame(({ clock }) => {
    ...
    const projected = (targets || []).map((target) => {
      ...
      const distance = Math.max(0.001, camera.position.distanceTo(world));
      const verticalWorld = 2 * Math.tan(THREE.MathUtils.degToRad(camera.fov || 43) / 2) * distance;
      const worldPerPixel = verticalWorld / Math.max(1, size.height);
      const radiusScene = Number(target.radiusAu) * Number(sceneUnitsPerAU);
      const projectedRadiusPx = Number.isFinite(radiusScene) && radiusScene > 0
        ? radiusScene / Math.max(0.000001, worldPerPixel)
        : null;
      const unresolved = !offscreen && projectedRadiusPx !== null && projectedRadiusPx < 14;
...
function SceneMotionMetrics({
  ...
  bodyContrastStarRadiusRatio = null,
  ...
  useEffect(() => {
    ...
    gl.domElement.dataset.trueOrbitMaxBodyToMinOrbitRatio = Number.isFinite(Number(trueOrbitMaxBodyToMinOrbitRatio))
      ...
      : "";
    gl.domElement.dataset.bodyContrastRadiusPolicy = normalizeScaleMode(scaleMode) === "true_bodies"
      ? "accepted-radius-or-class-proxy-v1"
      : "inactive";
    gl.domElement.dataset.bodyContrastStarRadiusRatio = Number.isFinite(Number(bodyContrastStarRadiusRatio))
      ? Number(bodyContrastStarRadiusRatio).toFixed(5)
      : "";
...
function PreviewObjects({ stars, planets, subsystems = [], renderOrbits = [], simulationTree = null, hierarchy, visualScale = DEFAULT_VISUAL_SCALE, scaleMode = "structure", running = true, speedMultiplier = 1, resetToken = 0, showOrbits = true, showHabitableZones = false, showFormationLines = DEFAULT_FORMATION_LINE_VISIBILITY, showLabels = true, selectedObjectId = "", targetRegistryRef = null, onHover, onSelect, onFocus, onClockSample }) {
  ...
  const bodyContrastStarRadiusRatio = useMemo(() => {
    if (activeScaleMode !== "true_bodies") return null;
    const radii = displayStars
      .map((star) => Number(star.display_radius_scene))
      .filter((radius) => Number.isFinite(radius) && radius > 0);
    if (radii.length < 2) return null;
    return Math.max(...radii) / Math.min(...radii);
  }, [activeScaleMode, displayStars]);
  ...
  return (
    <group>
      <SceneMotionMetrics
        ...
        bodyContrastStarRadiusRatio={bodyContrastStarRadiusRatio}
...
function SceneCanvas({ scene, scaleMode = "structure", running = true, speedMultiplier = 1, resetToken = 0, showOrbits = true, showHabitableZones = true, showFormationLines = DEFAULT_FORMATION_LINE_VISIBILITY, showLabels = true, selectedObjectId = "", focusKey = "", focusRequest = null, navigationTargets = [], keybindScheme = "wasd", transparentBackground = false, frameLoop = "always", preserveDrawingBuffer = true, qualityTier = "high", captureFrame = false, onFrameCapture = null, labelTypography = DEFAULT_SCENE_LABEL_TYPOGRAPHY, theme = "simple_dark", onHover, onSelect, onFocus, onPointerMissed, onClockSample, onContextLost, onScaleReport, onNavigationTargetsChange }) {
  ...
  const renderOrbits = useMemo(() => (scene?.render_scene?.orbits || []).map((orbit) => {
  ...
  const planets = useMemo(() => {
    ...
    if (renderPlanets.length) {
      return renderPlanets.map((planet, idx) => {
        ...
        const placement = physicalLabelPlacementForRecord(physicalLabelLayout, planet);
        return {
          ...planet,
  ...
  const orbitScaleDefinition = useMemo(
    () => orbitPresentationScaleDefinition(stars, planets, visualScale),
    [planets, stars, visualScale],
  );
  const reportedSceneUnitsPerAu = activeScaleMode === PHYSICAL_SCALE_MODE
    ? physicalUnitsPerAu
    : (activeScaleMode === "true_orbits" ? orbitScaleDefinition.localSceneUnitsPerAu : null);

  const subsystems = useMemo(() => (
    (scene?.render_scene?.bodies?.subsystems || []).map((subsystem, idx) => {
      const placement = physicalLabelPlacementForRecord(physicalLabelLayout, subsystem);
      return {
        ...subsystem,
  ...
  const compressedPhysicalRoot = useMemo(() => {
    ...
    const rootRadius = Number(focusGraph?.nodes?.[focusGraph.root_focus_key]?.physical_bounds?.radius_au);
    const childRadii = nodes
      .filter((node) => node.focus_key !== focusGraph.root_focus_key)
      .map((node) => Number(node.physical_bounds?.radius_au))
      .filter((value) => Number.isFinite(value) && value > 0);
    return Number.isFinite(rootRadius) && rootRadius > 0 && childRadii.some((value) => rootRadius / value >= 100);
  ...
  return (
    <SceneContrastContext.Provider value={contrast}>
      ...
      >
        ...
        <CameraControls resetToken={resetToken} scaleMode={renderScaleMode} focusRequest={focusRequest} targetRegistryRef={targetRegistryRef} keybindScheme={keybindScheme} />
        <ViewportScaleReporter
          enabled={[PHYSICAL_SCALE_MODE, "true_orbits"].includes(activeScaleMode)}
          sceneUnitsPerAU={reportedSceneUnitsPerAu}
        ...
        <SceneLabelTypographyContext.Provider value={labelTypography}>
          <PreviewObjects
            ...
            showLabels={showLabels && !compressedPhysicalRoot}
            selectedObjectId={selectedObjectId}
            targetRegistryRef={targetRegistryRef}
            onHover={onHover}
            onSelect={onSelect}
...
function ScaleOverlayClose({ onDismiss }) {
  return (
    <button
      type="button"
      className="system-preview-scale-dismiss"
      aria-label="Close scale information"
      title="Close scale information"
      onClick={onDismiss}
    >
      X
    </button>
...
function PhysicalScaleOverlay({ scaleMode, scaleReport, focusGraph, focusKey, modelAssisted = false, orbits = [], unavailableRequested = false, orbitScaleDefinition = null, onDismiss = null }) {
  ...
  if (!physical && !explainUnavailable) {
    if (scaleMode === "true_orbits") {
      const localReport = scaleReport?.mode === "true_orbits" ? scaleReport : null;
      const length = physicalScaleReadout(localReport?.lengthAu);
      const view = physicalScaleReadout(localReport?.viewWidthAu);
      return (
        <div className="system-preview-physical-ruler" data-testid="system-preview-local-ruler" data-scale-kind="local-orbit">
          <ScaleOverlayClose onDismiss={onDismiss} />
          <div className="system-preview-ruler-line" style={{ width: `${localReport?.widthPx || 90}px` }} />
          <strong>{length?.primary || "Measuring local view"}</strong>
          {length ? <span>{length.metric} / {length.lightTime}</span> : null}
          {view ? <small>View width {view.primary}</small> : null}
          <small>Planet and habitable zone spacing uses this local linear scale</small>
          <small>Stellar hierarchy distances remain schematic</small>
        </div>
      );
    }
    if (scaleMode === "log") {
      const ticks = logarithmicOrbitScaleTicks(orbitScaleDefinition);
      return (
        <div className="system-preview-log-scale" data-testid="system-preview-log-scale-key">
          <ScaleOverlayClose onDismiss={onDismiss} />
          <strong>Logarithmic planet and zone scale</strong>
          <div className="system-preview-log-scale-rail" aria-label="Logarithmic local orbit scale mapping">
            {ticks.map((tick, index) => (
              <span
                key={`${tick.fraction}:${tick.label}`}
                className="system-preview-log-scale-tick"
                data-edge={index === 0 ? "start" : (index === ticks.length - 1 ? "end" : "")}
          ...
          <small>Mapping key, not a viewport ruler</small>
          <small>Stellar hierarchy uses a separate presentation transform</small>
        </div>
      );
    }
    return (
      <div className="system-preview-scale-note" data-testid="system-preview-scale-note">
        <ScaleOverlayClose onDismiss={onDismiss} />
        {scaleMode === "true_bodies"
          ? "Body size contrast - radii are emphasized while distances remain presentation scaled"
          : "Schematic distance - presentation scaled for readability"}
      </div>
    );
  }
  const physicalReport = scaleReport?.mode === PHYSICAL_SCALE_MODE ? scaleReport : null;
  const length = physicalScaleReadout(physicalReport?.lengthAu);
  const view = physicalScaleReadout(physicalReport?.viewWidthAu);
  ...
  return (
    <div className="system-preview-physical-ruler" data-testid="system-preview-physical-ruler" data-physical-scale-status={unavailable ? "unavailable" : (resolution.inherited ? "inherited" : "available")}>
      ...
      {unavailable ? (
        ...
        <>
          <div className="system-preview-ruler-line" style={{ width: `${physicalReport?.widthPx || 90}px` }} />
...
function FocusNavigationOverlay({ focusGraph, focusKey, onFocusKey, onBack = null, canBack = false, topOffset = null }) {
  ...
  return (
    ...
    >
      ...
      <button type="button" onClick={() => previous && onFocusKey(previous)} disabled={!previous} aria-label="Previous focus region" title={navigationMode === "system-cycle" ? "Tour the previous region in this system" : "Open the previous focus region"}>Prev</button>
      ...
      <button type="button" onClick={() => next && onFocusKey(next)} disabled={!next} aria-label="Next focus region" title={navigationMode === "system-cycle" ? "Tour the next region in this system" : "Open the next focus region"}>Next</button>
    </nav>
  );
}

function OffscreenIndicators({ indicators = [], onSelect, onFocus }) {
  const visible = layoutBoundedIndicators(indicators, 5);
...
export default function SystemPreviewPanel({ systemId, systemName, snapshot = null, presentationMode = "detail", autoRun = true, qualityTier = "high", captureFrame = false, onFrameCapture = null, onRuntimeEvent = null, onStellarClassEntries = null, onPlanetCategoryEntries = null, onSceneLoaded = null, defaultScaleMode = "structure", nameStyle = "public_full", subjectTags = [], persistPresentationState = false }) {
  ...
  const [scaleInfoDismissed, setScaleInfoDismissed] = useState(false);
  const [cameraKeybindScheme, setCameraKeybindScheme] = useState(readStoredCameraKeybindScheme);
  ...
  const objectListRef = React.useRef(null);
  ...
  useEffect(() => {
    const handleKeybindChange = (event) => {
      setCameraKeybindScheme(normalizeCameraKeybindScheme(event?.detail?.scheme));
    };
    const handleStorage = (event) => {
      if (event.key === CAMERA_KEYBIND_STORAGE_KEY) {
        setCameraKeybindScheme(normalizeCameraKeybindScheme(event.newValue));
      }
    };
    window.addEventListener(CAMERA_KEYBIND_CHANGE_EVENT, handleKeybindChange);
    window.addEventListener("storage", handleStorage);
    return () => {
      window.removeEventListener(CAMERA_KEYBIND_CHANGE_EVENT, handleKeybindChange);
  ...
  const visualScale = renderScene.visual_scale || {};
  const orbitScaleDefinition = useMemo(
    () => orbitPresentationScaleDefinition(
      Array.isArray(renderBodies.stars) ? renderBodies.stars : [],
      Array.isArray(renderBodies.planets) ? renderBodies.planets : [],
      mergeVisualScale(visualScale),
    ),
    [scene],
  );
  ...
  const issueFocusRequest = useCallback((nextFocusKey, { enter = true, preserveDistance = false } = {}) => {
    ...
    if (enter) {
      if (activeFocusKey && activeFocusKey !== nextFocusKey) {
        setFocusHistory((current) => [...current.slice(-15), activeFocusKey]);
      }
  ...
  const returnToPreviousFocus = useCallback(() => {
    ...
    issueFocusRequest(previous, { enter: false });
    setFocusKey(previous);
    setFocusHistory((current) => current.slice(0, -1));
  }, [focusHistory, focusNodes, issueFocusRequest]);
  ...
  useEffect(() => {
    ...
    const current = list?.querySelector?.('[data-focus-current="true"]');
  ...
  const navigationTargets = useMemo(() => {
    ...
    return Object.values(focusNodes)
      .filter((node) => (
        ...
        && immediate.has(node.focus_key)
        && (node.object_key || node.orbit_key)
      ))
      .map((node) => {
        ...
        return {
          ...
          radiusAu: Number(node.physical_bounds?.radius_au) || null,
          priority,
        };
      })
    ...
    >
      ...
      <div className="system-preview-layout">
          ...
          {status === "fallback" || webglReady === false || contextFailureLocked
            ...
            ? (
              <SceneCanvas
                ...
                keybindScheme={cameraKeybindScheme}
                ...
                <>
                  ...
                  {!scaleInfoDismissed ? (
                    <PhysicalScaleOverlay
                      ...
                      orbitScaleDefinition={orbitScaleDefinition}
                      onDismiss={() => setScaleInfoDismissed(true)}
                    />
                  ) : null}
                </>
              ) : null}
        ...
        {normalizedPresentationMode !== "card" && <div className="system-preview-readout">
          ...
          />
          {showObjectList ? (
            <div ref={objectListRef} className="system-preview-object-list" data-testid="system-preview-object-list">
              ...
              {objectItems.length > 0 ? objectItems.map((item) => {
                ...
                const itemFocusKey = focusKeyForObject(item.payload);
                const focusCurrent = Boolean(itemFocusKey && itemFocusKey === activeFocusKey);
                return (
                <div
                  key={item.key}
                  className={`system-preview-object-chip${focusCurrent ? " is-focus-current" : ""}`}
