...
def _accepted_field(field: Dict[str, Any]) -> bool:
    return (
        ...
        and str(field.get("status") or "").lower() in {"source", "derived"}
    )
...
def _axis_coherence(
    ...
    period_value = _positive_float(period.get("value")) if _accepted_field(period) else None
...
def attach_physical_orbit_extent(
    ...
    *,
    ...
    elif _accepted_field(period) and _positive_float(total_mass_msun) is not None:
        axis_value = kepler_semi_major_axis_au(period.get("value"), total_mass_msun)
        mass_statuses = {
            str(item.get("status") or "missing").lower()
            for item in mass_inputs
            if isinstance(item, dict)
        }
        confidence = 0.82 if mass_statuses and mass_statuses <= {"source"} else 0.68
...
def _body_mass(body: Dict[str, Any]) -> Optional[float]:
    ...
    if str(field.get("status") or "").lower() not in {"source", "derived"}:
        return None
...
def attach_physical_extents_to_orbits(
    ...
    for orbit in orbits:
        ...
        masses = [_body_mass(stars_by_key[key]) for key in keys]
